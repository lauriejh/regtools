...
#' @param data_type Demographic data can either be of type "t_unvarying" or "t_varying", necessary to check correct data structure characteristics.
...
read_demo_data <- function(file_path, data_type, id_col = "id", date_col = "date", ...) {
  ...
  ###### Time dependent: Check date column #####
  if (data_type == "time_dependent"){
    message("Data type: time-dependent. Checking requirements...")
  ...
  ###### Persistent: check ID duplicates #####
  if (data_type == "persistent"){
    message("Data type: persistent. Checking requirements...")
