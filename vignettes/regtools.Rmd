...
<!-- Add information about the other functions, and link to them. Just one line -->
...
To keep only cases (identified by an unique person identifier) with at least two diagnostic events, but summarizing information by their first-time diagnosis: 
...
```
 ...
 In the case of time-invariant data, to keep only individuals with year of birth between 2006-2008 and reason of immigration "ARB", "NRD" or "UKJ": 
...
The `link_diag_demo()` function can aid with the linkage process conditioned that all datasets share the same IDs to uniquely idetify individuals. To link both the already filtered diagnostic and time-invariant datasets: 

```{r link-data, echo = TRUE}
linked_diag_inv <- link_diag_demo(
  data_diag = filtered_diag_pattern,
...
Until this point, all the functions described are (can be used... Regardless of analysis, data preparation in general.) Considering a lot of this type of data is used for epidemiology and monitoring, package also includes some more directed function to aid with calculating prevalence and incidence. 
